...
compute_CV1 <- function(y, k, b1) {
  ...
  for (t in (k + 2):N) {
    ...
    e_hat[t] <- dy[t] - x_t %*% coef(fit)
  }
  
  CV1 <- mean(e_hat^2, na.rm = TRUE)
  return(CV1)
}

compute_CV2 <- function(y, k, b1, b2, sd0, p) {
  N <- length(y)
  dy <- c(NA, diff(y))
  i <- seq(N)
  ...
  CV2 <- mean((e_trunc^2 - var_hat)^2, na.rm = TRUE)
  return(CV2)
}
...
b2_scores_btc <- sapply(b2_grid, function(x) 
  compute_CV2(data$btc, k = 0, b1 = b1_opt_btc, b2 = x, sd0 = sd0$btc, p = 7))
...
b2_scores_eth <- sapply(b2_grid, function(x) 
  compute_CV2(data$eth, k = 0, b1 = b1_opt_eth, b2 = x, sd0 = sd0$eth, p = 7))
...
b2_scores_ltc <- sapply(b2_grid, function(x) 
  compute_CV2(data$ltc, k = 0, b1 = b1_opt_ltc, b2 = x, sd0 = sd0$ltc, p = 10))
